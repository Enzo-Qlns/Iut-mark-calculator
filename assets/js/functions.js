...
function createCardBody(content, title, colLength = 6) {
  ...
  if (content instanceof HTMLElement) card_body.appendChild(content);
  else card_body.innerHTML = content;
...
function createChart(data, type, xaxiscategories) {
  const options = {
    ...
    colors: [
      function ({ value }) {
        if (value < 10) return "#f96868";
        else if (value <= 12) return "#faa64b";
        else return "#15c377";
      },
    ],
    plotOptions: {
 ...
 */
function getAverage() {
  const listNote = document.querySelectorAll(
    "#mainContent > div.row > div:nth-child(3) > div > div > table > tbody tr"
  );
  const listModal = document.querySelectorAll(
    "#mainContent > div.row > div:nth-child(4) > div > div > table > tbody tr"
  );

  const notesData = {};
  for (const elt of listNote) {
    let nameMoy = elt.children[0].textContent.trim();
    let note = Number.parseFloat(
      elt.children[4].children[0].textContent.replace(",", ".")
    );
...
function generateHtml(averageDataByUE) {
  ...
  for (const [domaine, note] of Object.entries(averageDataByUE)) {
    ...
    td.innerHTML = `<span class="fs-11 badge ${
      parseFloat(note) < 10
        ? "bg-danger"
        : parseFloat(note) <= 12
        ? "bg-warning"
        : "bg-success"
    }">${Utils.roundValue(note, 2)}</span>`;
    trBody.append(td);
  }
  tbody.append(trBody);
  ...
  olIsAccepted.className =
    "timeline timeline-activity timeline-point-sm timeline-content-right text-left w-100";
  const liIsAccepted = document.createElement("li");
  liIsAccepted.className = "alert alert-" + (isAccepted ? "success" : "danger");
  liIsAccepted.innerHTML =
    '<strong class="fw-semibold">Validation : </strong> ' +
    Utils.boolToValue(isAccepted);
  olIsAccepted.append(liIsAccepted);
  const isAcceptedHtml = createCardBody(
    olIsAccepted,
...
function toastPaypal(callBack) {
  Swal.fire({
    title:
      "<span style='font-weight: bold; font-size: 40px !important;'>Juste une seconde !</span>",
    ...
    if (result.isConfirmed) {
      window.open(
        "https://www.paypal.com/donate/?hosted_button_id=UM5G4CD7PTJQJ",
        "_blank"
      );
    }
    callBack();
